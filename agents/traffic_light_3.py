...
async def handle_controller_message(ctx: Context, sender: str, msg: ControllerMessage):
    ctx.logger.info(f"Traffic Light 3 (WEST - straight only) received command: {msg.color}")
...
if __name__ == "__main__":
    ...
    if not Path("light3.json").exists():
